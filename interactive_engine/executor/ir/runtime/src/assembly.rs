...
use graph_proxy::apis::{register_graph, get_process_partition_lists, get_server_index, replace_read_version, get_read_version, replace_grin_partitioned_graph, get_grin_partitioned_graph, replace_grin_graph, get_grin_graph};
...
use grin::grin::grin_destroy_partitioned_graph;
use grin::grin::grin_destroy_graph;
...
impl<P: PartitionInfo, C: ClusterInfo> JobAssembly<Record> for IRJobAssembly<P, C> {
    fn assemble(&self, plan: &JobDesc, worker: &mut Worker<Record, Vec<u8>>) -> Result<(), BuildJobError> {
        worker.dataflow(move |input, output| {
            ...
            // read the read_epoch from /tmp/read_epoch file and convert it to u32
            let read_epoch = std::fs::read_to_string("/tmp/read_epoch").unwrap().parse::<u32>().unwrap();
            let prevous_read_epoch = get_read_version();
            if prevous_read_epoch != read_epoch {
                replace_read_version(read_epoch.clone());
                unsafe {  
                    let previous_pg = get_grin_partitioned_graph();
                    if !previous_pg.is_null() {
                        grin_destroy_partitioned_graph(previous_pg);
                    }
                    let grin_graph = get_grin_graph();
                    if !grin_graph.is_null() {
                    ...
                    replace_grin_partitioned_graph(pg);        
